...
type RegisterClusterInPushOption struct {
...
func RegisterClusterInPushMode(opts *RegisterClusterInPushOption) error {
	registerOption := karmadautil.ClusterRegisterOption{
		...
		ControlPlaneConfig: opts.controlPlaneRestConfig,
		ClusterConfig:      opts.memberClusterRestConfig,
	}
	controlPlaneKubeClient := kubeclient.NewForConfigOrDie(opts.controlPlaneRestConfig)
...
func RegisterClusterInPullMode(opts *RegisterClusterInPullOption) error {
	_, exist, err := karmadautil.GetClusterWithKarmadaClient(opts.karmadaClient, opts.clusterName)
	if err != nil {
		return err
	}
	if exist {
		return fmt.Errorf("failed to register as cluster with name %s already exists", opts.clusterName)
	}
...
type RegisterClusterInPullOption struct {
	karmadaClient          karmadaclientset.Interface
...
func (o RegisterClusterInPullOption) createSecretAndRBACInMemberCluster() error {
...
func (o RegisterClusterInPullOption) makeKarmadaAgentDeployment() *appsv1.Deployment {
...
/*
	// 解析 kubeconfig 内容
	memberClusterApiConfig, err := clientcmd.Load([]byte(memberClusterKubeConfig))
	if err != nil {
		// return nil, err
	}
	// 创建一个包含 kubeconfig 内容的 ClientConfig 对象
	memberClusterClientConfig := clientcmd.NewDefaultClientConfig(*memberClusterApiConfig, &clientcmd.ConfigOverrides{})
	memberClusterRestConfig, err := memberClusterClientConfig.ClientConfig()
	if err != nil {

	}

	clusterName := "test-cluster"
	memberClusterKubeClient := kubeclient.NewForConfigOrDie(memberClusterRestConfig)
	...
	registerOption := karmadautil.ClusterRegisterOption{
		...
		ClusterConfig:      memberClusterRestConfig,
	}
*/

func KubeRestConfigFromKubeConfig(kubeconfig string) (*rest.Config, error) {
...
func KubeClientSetFromKubeConfig(kubeconfig string) (*kubeclient.Clientset, error) {
	restConfig, err := KubeRestConfigFromKubeConfig(kubeconfig)
	if err != nil {
		return nil, err
	}
