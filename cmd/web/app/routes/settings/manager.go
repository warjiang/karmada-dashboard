...
func (sm *SettingsManager) load(client kubernetes.Interface) (changed bool) {
	...
	configMap, err := client.CoreV1().ConfigMaps(opts.ArgNamespace).
		Get(context.Background(), opts.ArgSettingsConfigMapName, metav1.GetOptions{})
	if err != nil {
...
func (sm *SettingsManager) patchConfigMap(client kubernetes.Interface, key, value string) error {
	...
	_, err = client.CoreV1().ConfigMaps(opts.ArgNamespace).
		Patch(context.Background(), opts.ArgSettingsConfigMapName, types.MergePatchType, patch, metav1.PatchOptions{})
	return err
}
